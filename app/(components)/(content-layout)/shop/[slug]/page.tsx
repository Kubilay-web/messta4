...
type PaymentMethod = "cod_cash" | "cod_card" | "stripe" | "paypal";
...
const ProductDetail = () => {
  ...
  const { t, money, currency } = useShopSettings();
  ...
  const [paymentMethod, setPaymentMethod] = useState<PaymentMethod>("cod_cash");
  ...
  const PAYMENTS: {
  ...
  }[] = [
    {
      key: "cod_cash",
      label: t("pm_cod_cash"),
      desc: t("pm_cod_cash_desc"),
      icon: "ri-money-dollar-circle-line",
    },
    {
      key: "cod_card",
      label: t("pm_cod_card"),
      desc: t("pm_cod_card_desc"),
      icon: "ri-bank-card-2-line",
    },
    {
      key: "stripe",
      label: t("pm_stripe"),
      desc: t("pm_stripe_desc"),
      icon: "ri-bank-card-line",
    },
    {
      key: "paypal",
      label: t("pm_paypal"),
      desc: t("pm_paypal_desc"),
      icon: "ri-paypal-line",
    },
  ];
  ...
  const submitOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Kapıda ödeme: yönlendirme yok, siparişlerim sayfasına git
      if (data.cod) {
        toast.success(t("orderPlacedCod"));
        window.location.href = data.redirectUrl || "/shop/orders?order_success=1";
        return;
      }

      // Online ödeme: sağlayıcıya yönlendir
      if (!data.paymentUrl) throw new Error(data.error || "Ödeme başlatılamadı");
      ...
      <div className="flex flex-col gap-4 p-4 sm:p-6 max-w-[1400px] mx-auto w-full">
        ...
        <div>
          <Link href="/shop" className="ti-btn ti-btn-light ti-btn-sm">
            <i className="ri-arrow-left-line me-1"></i> {t("backToStore")}
          </Link>
        </div>
        ...
        {loading && (
          <div className="flex items-center justify-center py-20">
            ...
            <span className="ms-3 text-muted">{t("productLoading")}</span>
          </div>
        )}
        ...
        {!loading && !product && (
          <div className="box">
            <div className="box-body flex flex-col items-center justify-center text-center gap-4 py-16">
              ...
              <p className="text-muted mb-0">{error || t("productNotFound")}</p>
          ...
          <div className="flex flex-col gap-y-6">
                    ...
                    <div>
                      {product.stock > 0 ? (
                        <span className="badge bg-success/10 text-success">
                          ...
                          {t("inStockN")} ({product.stock} {t("pieces")})
                        </span>
                      ) : (
            ...
            <div id="siparis" className="box">
              <div className="box-header">
                <h5 className="box-title">{t("orderInfo")}</h5>
              </div>
              <div className="box-body">
                ...
                <form onSubmit={submitOrder} className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col md:flex-row gap-4">
                    ...
                    <div className="flex-1 flex flex-col gap-1">
                      <label className="form-label mb-0">{t("city")} *</label>
                      <input
                        name="city"
                        value={form.city}
                        onChange={handleChange}
                  ...
                  <div className="flex flex-col gap-1">
                    <label className="form-label mb-0">{t("orderNote")}</label>
                    <textarea
                      name="note"
                      value={form.note}
                      onChange={handleChange}
                    ...
                    <div className="flex flex-wrap gap-3">
                      {PAYMENTS.map((opt) => (
                        <button
                          ...
                          className={`flex-1 min-w-[12rem] flex items-start gap-3 border rounded-md px-4 py-3 text-start transition-all ${
                        ...
                        >
                          <i
                            className={`${opt.icon} text-xl ${
                              paymentMethod === opt.key ? "text-primary" : ""
                            }`}
                          ></i>
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2 border-t border-defaultborder dark:border-defaultborder/10">
                    <div className="flex items-center gap-2">
                      <span className="text-muted">{t("payableAmount")}</span>
                    ...
                    >
                      {submitting ? (
                        isCod ? t("placingOrder") : t("redirecting")
                      ) : (
                        <>
